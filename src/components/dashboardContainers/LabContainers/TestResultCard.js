...
class TestResultCards extends Component {
  constructor(props) {
    super(props);
    this.state = {
      file: ""
  ...
  openFileHandler = testId => {
    const postId = testId;
    axios
      .get(`/v1/uploading/labUpload/${postId}`)
      .then(res => {
        console.log(res.data.post);
        this.setState({
          file: "http://localhost:3001/" + res.data.post.fileURL
        });
      })
      .catch(err => {
        console.log(err);
            ...
            >
              <div className="generating-cap" style={{ padding: "20px" }}>
                ...
                <Button
                  ...
                  color="purple"
                  onClick={() => {
                    this.openFileHandler(this.props.testId);
                  }}
                >
                  view file
                </Button>
